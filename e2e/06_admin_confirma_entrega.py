# 06 - administrador confirma que o produto foi ENTREGUE
# admin loga no painel, vai na aba pedidos e clica em "Confirmar Entrega"
...
def test_admin_confirma_entrega(page: Page):
    """admin loga, abre painel de pedidos e confirma entrega."""
    ...
    # clica no botao "Confirmar Entrega" do pedido especifico
    page.locator(f'button[onclick*="updateOrderStatus({compra_id}"]:has-text("Confirmar Entrega")').click()
    expect(page.locator(".toast").last).to_contain_text("confirmar entrega", timeout=10000)

    # verifica que o status mudou
    page.wait_for_timeout(2000)
    row = page.locator(f"tr.admin-order-row", has=page.locator(f"td:text-is('#{compra_id}')"))
    expect(row.locator(".badge-status.status-delivered")).to_be_visible()

    salvar("compra_status", "ENTREGUE")
