# 04 - administrador confirma o pagamento (aprova a compra)
# admin loga no painel, vai na aba pedidos e clica em "Aprovar"
...
def test_admin_confirma_pagamento(page: Page):
    """admin loga, abre painel de pedidos e aprova a compra."""
    ...
    # clica no botao "Aprovar" do pedido especifico
    page.locator(f'button[onclick*="updateOrderStatus({compra_id}"]:has-text("Aprovar")').click()
    expect(page.locator(".toast").last).to_contain_text("aprovar", timeout=10000)

    # verifica que o status mudou
    page.wait_for_timeout(2000)
    row = page.locator(f"tr.admin-order-row", has=page.locator(f"td:text-is('#{compra_id}')"))
    expect(row.locator(".badge-status.status-approved")).to_be_visible()

    salvar("compra_status", "APROVADA")
